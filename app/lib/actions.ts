...
export async function createInvoice(prevState: State, formData: FormData) {
  ...
  //   const date = new Date().toLocaleString('en-NZ').split(',')[0];
...
export async function updateInvoice(
  ...
  // Call redirect after the try/catch block, as redirect works by throwing an error.
  // It would only be reachable it the try is successful
  revalidatePath('/dashboard/invoices');
  redirect('/dashboard/invoices');
}
